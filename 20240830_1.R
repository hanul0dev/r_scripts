...
#벡터 개수가 다를 때는 개수가 적은 쪽의 인덱스를 반복해서 연산한다.
x+w
...
#벡터에 잇는 요소의 값들중에 하나라도 비교연산 결과에 true가 있으면 결과는 true
any(x<=10)
...
#요인(factor)
# 벡터의 저장된 동일한 값들을 요인으로 찾아내서 대표값들을 분류해서 반환 + 원래 벡터 값들
